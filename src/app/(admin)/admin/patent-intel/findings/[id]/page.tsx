...
export default function FindingDetailPage() {
  ...
  if (loading) return <div style={{ minHeight: '100vh', background: 'var(--bg-primary)', color: 'var(--text-muted)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>Loading...</div>
  if (!finding) return <div style={{ minHeight: '100vh', background: 'var(--bg-primary)', color: '#ef4444', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>Finding not found</div>
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg-primary)', color: 'var(--text-primary)', fontFamily: 'system-ui, sans-serif', padding: 24 }}>
      <div style={{ maxWidth: 800, margin: '0 auto' }}>
        ...
        <div style={{ marginTop: 16, background: 'var(--bg-card)', borderRadius: 16, padding: 28, border: '1px solid var(--border)' }}>
          {/* Header */}
          <div style={{ display: 'flex', gap: 10, marginBottom: 16, flexWrap: 'wrap' }}>
            ...
            <span style={{ padding: '4px 12px', borderRadius: 6, fontSize: 11, fontWeight: 600, background: 'var(--bg-card)', color: '#8b9ab5', textTransform: 'uppercase' }}>
              {finding.category.replace(/_/g, ' ')}
            </span>
            <span style={{ padding: '4px 12px', borderRadius: 6, fontSize: 11, fontWeight: 600, background: 'var(--bg-card)', color: '#8b9ab5' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 20 }}>
            {[
              ...
              <div key={d.label} style={{ padding: 12, borderRadius: 8, background: 'var(--bg-primary)', border: '1px solid var(--border)' }}>
                <div style={{ fontSize: 10, color: 'var(--text-muted)', fontWeight: 700, textTransform: 'uppercase', marginBottom: 4 }}>{d.label}</div>
          ...
          {finding.overlapping_patents?.length ? (
            <div style={{ padding: 16, borderRadius: 10, background: 'rgba(245,158,11,0.06)', border: '1px solid rgba(245,158,11,0.15)', marginBottom: 20 }}>
              ...
              <div style={{ fontSize: 13, color: 'var(--text-primary)', marginBottom: 4 }}>Affected applications: {finding.overlapping_patents.join(', ')}</div>
          ...
          {finding.source_url && (
            <a href={finding.source_url} target="_blank" rel="noopener" style={{
              ...
              background: 'var(--bg-card)', color: '#6EE05A', fontSize: 13, fontWeight: 600, textDecoration: 'none',
            }}>
              View Source →
            </a>
          )}
          ...
          <div style={{ borderTop: '1px solid #1e293b', paddingTop: 20 }}>
            <h3 style={{ fontSize: 14, fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', marginBottom: 12 }}>Review</h3>

            {finding.reviewed && finding.review_status && (
              <div style={{ marginBottom: 12, padding: 12, borderRadius: 8, background: 'var(--bg-primary)', border: '1px solid var(--border)' }}>
