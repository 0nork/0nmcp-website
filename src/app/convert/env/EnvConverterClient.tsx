...
export default function EnvConverterClient() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#060a0f', color: '#e2e8f0' }}>
      <header style={{ padding: '1rem 2rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
      ...
      <section style={{ textAlign: 'center', padding: '3rem 1rem 2rem', maxWidth: 720, margin: '0 auto' }}>
        ...
        <h1 style={{ fontSize: 'clamp(1.75rem,4vw,2.75rem)', fontWeight: 800, color: '#fff', lineHeight: 1.15, margin: '0 0 1rem' }}>Convert <span style={{ color: '#ef4444' }}>.env</span> to <span style={{ color: '#7ed957' }}>.0n</span></h1>
        <p style={{ color: '#94a3b8', fontSize: '1.05rem', lineHeight: 1.6, maxWidth: 560, margin: '0 auto' }}>Your API keys are sitting in plain text. Paste your .env below and get encrypted .0n files with 7-layer security. <strong style={{ color: '#cbd5e1' }}>Your keys never leave your browser.</strong></p>
      </section>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: converted ? '1fr 1fr' : '1fr', gap: '1.5rem', transition: 'all 0.3s ease' }}>
          <div>
            <div style={{ background: '#0d1117', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 12, overflow: 'hidden' }}>
              <div style={{ padding: '0.75rem 1rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'rgba(255,255,255,0.03)', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  ...
                  <span style={{ fontSize: '0.75rem', color: '#64748b', marginLeft: 8 }}>.env</span>
                </div>
                <div style={{ display: 'flex', gap: '0.5rem' }}>
                  <button onClick={() => { setInput(EXAMPLE.replace(/\\n/g, '\n')); setConverted(false) }} style={{ padding: '4px 10px', borderRadius: 4, border: '1px solid rgba(255,255,255,0.1)', background: 'transparent', color: '#94a3b8', fontSize: '0.7rem', cursor: 'pointer' }}>Load Example</button>
                  <button onClick={() => { setInput(''); setConverted(false); setGroups([]) }} style={{ padding: '4px 10px', borderRadius: 4, border: '1px solid rgba(255,255,255,0.1)', background: 'transparent', color: '#94a3b8', fontSize: '0.7rem', cursor: 'pointer' }}>Clear</button>
                </div>
              </div>
              <textarea value={input} onChange={e => { setInput(e.target.value); setConverted(false) }} placeholder={'# Paste your .env file here\nSTRIPE_SECRET_KEY=sk_live_...\nOPENAI_API_KEY=sk-proj-...\nDATABASE_URL=postgresql://...'} spellCheck={false} style={{ width: '100%', minHeight: 360, padding: '1rem', background: 'transparent', border: 'none', outline: 'none', color: '#e2e8f0', fontFamily: "'JetBrains Mono',monospace", fontSize: '0.8rem', lineHeight: 1.7, resize: 'vertical' }} />
            </div>
          ...
          {converted && (
            <div>
              ...
              <div style={{ display: 'flex', gap: '0.25rem', marginBottom: '0.5rem', flexWrap: 'wrap' }}>
                <button onClick={() => setActiveTab('switch')} style={{ padding: '6px 12px', borderRadius: 6, border: 'none', background: activeTab === 'switch' ? 'rgba(126,217,87,0.2)' : 'rgba(255,255,255,0.05)', color: activeTab === 'switch' ? '#7ed957' : '#64748b', fontSize: '0.7rem', fontWeight: 600, cursor: 'pointer' }}>master.0n</button>
                {groups.map(g => (<button key={g.service} onClick={() => setActiveTab(g.service)} style={{ padding: '6px 12px', borderRadius: 6, border: 'none', background: activeTab === g.service ? g.color + '22' : 'rgba(255,255,255,0.05)', color: activeTab === g.service ? g.color : '#64748b', fontSize: '0.7rem', fontWeight: 600, cursor: 'pointer' }}>{g.service}.0n</button>))}
              </div>
              <div style={{ background: '#0d1117', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 12, overflow: 'hidden' }}>
                <div style={{ padding: '0.75rem 1rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'rgba(255,255,255,0.03)', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                  ...
                  <button onClick={handleCopy} style={{ padding: '4px 12px', borderRadius: 4, background: copied ? 'rgba(126,217,87,0.2)' : 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', color: copied ? '#7ed957' : '#94a3b8', fontSize: '0.7rem', fontWeight: 600, cursor: 'pointer' }}>{copied ? 'Copied' : 'Copy'}</button>
                </div>
                <pre style={{ padding: '1rem', margin: 0, overflow: 'auto', maxHeight: 340, fontFamily: "'JetBrains Mono',monospace", fontSize: '0.75rem', lineHeight: 1.6, color: '#e2e8f0' }}>{activeTab === 'switch' ? genSwitch(groups) : genConnection(groups.find(g => g.service === activeTab)!)}</pre>
              </div>
              <div style={{ marginTop: '1.25rem', padding: '1.25rem', borderRadius: 12, background: 'rgba(126,217,87,0.06)', border: '1px solid rgba(126,217,87,0.15)', textAlign: 'center' }}>
                <p style={{ color: '#cbd5e1', fontSize: '0.9rem', margin: '0 0 0.75rem', fontWeight: 600 }}>Save to your encrypted vault</p>
      ...
      <section style={{ maxWidth: 700, margin: '0 auto', padding: '2rem 1rem 4rem' }}>
        <h2 style={{ fontSize: '1.5rem', fontWeight: 800, color: '#fff', textAlign: 'center', marginBottom: '2rem' }}><span style={{ color: '#ef4444' }}>.env</span> vs <span style={{ color: '#7ed957' }}>.0n</span></h2>
        <div style={{ background: '#0d1117', borderRadius: 12, border: '1px solid rgba(255,255,255,0.08)', overflow: 'hidden' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.85rem' }}>
            <thead><tr style={{ borderBottom: '1px solid rgba(255,255,255,0.08)' }}><th style={{ padding: '0.875rem 1rem', textAlign: 'left', color: '#64748b', fontWeight: 600 }}>Feature</th><th style={{ padding: '0.875rem 1rem', textAlign: 'center', color: '#ef4444', fontWeight: 600 }}>.env</th><th style={{ padding: '0.875rem 1rem', textAlign: 'center', color: '#7ed957', fontWeight: 600 }}>.0n</th></tr></thead>
            <tbody>
              {[['Encryption','None','AES-256-GCM'],['Key derivation','None','Argon2id'],['Service detection','No','20+ services'],['Cross-platform','No','7 AI platforms'],['Access control','No','Per-layer'],['Tamper detection','No','Seal of Truth'],['Version control safe','No','Yes'],['Digital signatures','No','Ed25519'],['Multi-party sharing','No','X25519 escrow'],['Patent protection','No','5 patents pending']].map(([f,e,o],i) => (
                <tr key={i} style={{ borderBottom: '1px solid rgba(255,255,255,0.04)' }}><td style={{ padding: '0.75rem 1rem', color: '#cbd5e1' }}>{f}</td><td style={{ padding: '0.75rem 1rem', textAlign: 'center', color: '#94a3b8' }}>{e}</td><td style={{ padding: '0.75rem 1rem', textAlign: 'center', color: '#7ed957', fontWeight: 600 }}>{o}</td></tr>
              ))}
            </tbody>
          </table>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.75rem', marginTop: '2rem', color: '#475569', fontSize: '0.8rem' }}>
