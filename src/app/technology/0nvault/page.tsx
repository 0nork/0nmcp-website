    ...
    <div className="pt-28 pb-24 px-4 md:px-8">
      <div className="max-w-4xl mx-auto">
        ...
        <nav className="text-xs mb-4 flex items-center gap-1.5" style={{ color: 'var(--text-muted)' }} aria-label="Breadcrumb">
          ...
          <span>/</span>
          <span style={{ color: '#a78bfa' }}>0nVault</span>
        </nav>

        {/* Hero */}
        <div className="heading-glow">
          <span className="text-xs font-mono uppercase tracking-widest mb-3 block" style={{ color: '#a78bfa' }}>
        ...
        <section className="rounded-2xl p-6 md:p-8 mb-10" style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border)' }}>
          ...
          <p className="text-sm leading-relaxed mb-4" style={{ color: 'var(--text-secondary)' }}>
            Imagine a safe deposit box at a bank — but instead of one compartment, it has <strong style={{ color: 'var(--text-primary)' }}>7 separate compartments</strong>, each with its own key.
          </p>
          <p className="text-sm leading-relaxed mb-4" style={{ color: 'var(--text-secondary)' }}>
        ...
        <section className="mb-10">
          ...
          <div className="space-y-3">
            {LAYERS.map((layer, i) => (
              <div key={layer.name} className="flex items-start gap-4 rounded-xl p-5" style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border)' }}>
                <span className="text-2xl shrink-0">{layer.icon}</span>
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-xs font-mono" style={{ color: 'var(--text-muted)' }}>Layer {i + 1}</span>
                    <h3 className="font-bold text-sm" style={{ color: '#a78bfa' }}>{layer.name}</h3>
                    {layer.name === 'Credentials' && (
                      <span className="text-[10px] px-2 py-0.5 rounded-full" style={{ backgroundColor: 'rgba(255,107,53,0.15)', color: '#ff6b35', border: '1px solid rgba(255,107,53,0.3)' }}>
                        Double Encrypted
                      </span>
                    )}
                  </div>
        ...
        <section className="mb-10">
          ...
          <div className="grid sm:grid-cols-2 gap-5">
            {[
              ...
              <div key={f.title} className="rounded-xl p-5" style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border)' }}>
        ...
        <section className="rounded-2xl p-6 md:p-8 mb-10" style={{ background: 'linear-gradient(135deg, rgba(167,139,250,0.06), rgba(0,212,255,0.04))', border: '1px solid rgba(167,139,250,0.15)' }}>
          <h2 className="text-xl font-bold mb-4" style={{ color: 'var(--text-primary)' }}>
            Who uses this?
          </h2>
          <div className="space-y-3">
            {[
              ...
              <div key={uc.who} className="flex gap-3">
                ...
                <div>
                  <strong className="text-sm" style={{ color: 'var(--text-primary)' }}>{uc.who}:</strong>{' '}
                  <span className="text-sm" style={{ color: 'var(--text-secondary)' }}>{uc.use}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <section className="mb-10">
          ...
          <div className="grid sm:grid-cols-3 gap-4">
            ...
            <Link href="/security/vault" className="rounded-xl p-4 no-underline group" style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border)' }}>
              <h3 className="font-bold text-sm mb-1 group-hover:underline" style={{ color: '#a78bfa' }}>Technical Specs</h3>
              <p className="text-xs" style={{ color: 'var(--text-muted)' }}>AES-256-GCM, Ed25519, binary format details</p>
            </Link>
            <Link href="/security/escrow" className="rounded-xl p-4 no-underline group" style={{ backgroundColor: 'var(--bg-card)', border: '1px solid var(--border)' }}>
              <h3 className="font-bold text-sm mb-1 group-hover:underline" style={{ color: '#00d4ff' }}>Multi-Party Escrow</h3>
              <p className="text-xs" style={{ color: 'var(--text-muted)' }}>How key agreement works between parties</p>
            </Link>
          </div>
        </section>
