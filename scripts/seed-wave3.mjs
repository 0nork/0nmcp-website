#!/usr/bin/env node
/**
 * 0nMCP Forum Seeder — Wave 3 (SEO-Targeted)
 * Reads seed-wave3a-data.json and seed-wave3b-data.json, inserts threads + replies into Supabase.
 * Targets: "MCP integration app", "SXO / Search Experience Optimization", "agentic AI bots"
 *
 * Usage: NODE_TLS_REJECT_UNAUTHORIZED=0 node scripts/seed-wave3.mjs
 */
...
function randomViewCount() {
  return Math.floor(Math.random() * 281) + 30 // 30-310 (slightly higher for SEO threads)
}

// ==================== Seed one data file ====================

async function seedFile(filePath, label) {
  console.log(`\n--- Seeding: ${label} ---\n`)
  ...
  for (let i = 0; i < data.threads.length; i++) {
    ...
    threadCount++
    console.log(`  + [${t.group}] "${t.title.slice(0, 60)}" by ${t.author}`)
  }
  ...
  for (let i = 0; i < data.replies.length; i++) {
    ...
    console.log(`  + ${r.author} -> "${threadTitle.slice(0, 50)}"`)
  }
  ...
  for (const [threadId, count] of Object.entries(replyCounts)) {
    const updateData = { reply_count: count }
  ...
  console.log(`  Updated ${updateCount} thread reply counts\n`)
  ...
  for (const slug of allPersonas) {
    ...
    if (lookupErr || !persona) {
      console.log(`  SKIP persona "${slug}": not found`)
      continue
    }
    ...
    const { error: updateErr } = await db
      .from('community_personas')
      .update({
        thread_count: (persona.thread_count || 0) + threads,
        reply_count: (persona.reply_count || 0) + replies,
  ...
  console.log(`  Updated ${personaUpdateCount} personas\n`)

  return { threadCount, replyCount, updateCount, personaUpdateCount }
}
...
async function main() {
  console.log('\n========================================')
  console.log('  0nMCP Forum Seeder — Wave 3 (SEO)')
  console.log('  Targets: MCP integration app, SXO,')
  console.log('  agentic AI bots')
  console.log('========================================\n')

  const results = []

  // Seed Wave 3A: MCP integration app + SXO
  const file3a = join(__dirname, 'seed-wave3a-data.json')
  try {
    results.push(await seedFile(file3a, 'Wave 3A — MCP Integration App + SXO'))
  } catch (err) {
    console.error('ERROR loading Wave 3A:', err.message)
  }

  // Seed Wave 3B: Agentic AI
  const file3b = join(__dirname, 'seed-wave3b-data.json')
  try {
    results.push(await seedFile(file3b, 'Wave 3B — Agentic AI Bots'))
  } catch (err) {
    console.error('ERROR loading Wave 3B:', err.message)
  }

  // ---- Summary ----
  const totals = results.reduce((acc, r) => ({
    threads: acc.threads + r.threadCount,
    replies: acc.replies + r.replyCount,
    threadUpdates: acc.threadUpdates + r.updateCount,
    personaUpdates: acc.personaUpdates + r.personaUpdateCount,
  }), { threads: 0, replies: 0, threadUpdates: 0, personaUpdates: 0 })

  console.log('========================================')
  console.log('  WAVE 3 SEEDING COMPLETE')
  console.log('========================================')
  console.log(`  Total threads:   ${totals.threads}`)
  console.log(`  Total replies:   ${totals.replies}`)
  console.log(`  Thread updates:  ${totals.threadUpdates}`)
  console.log(`  Persona updates: ${totals.personaUpdates}`)
  console.log('')
}
